...
def update_market_order(session, order_id, status, close_amount = 0):
    market = session.query(Market).filter_by(order_id=order_id).first()
    market.status = status
    market.close_amount = close_amount
    session.commit()
...
def get_all_open_orders(session):
    return session.query(Market).filter(and_(Market.deleted_at.is_(None), and_(Market.status != "closed", Market.status != "canceled", Market.status != "failed"))).all()
